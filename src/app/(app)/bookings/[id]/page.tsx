...
export default function BookingDetailPage() {
  ...
  const merged = useMemo(() => {
    const fee =
      Number(booking?.base_fee ?? 0) +
      Number(booking?.travel_fee_estimate ?? 0);
    ...
    const total_time_mins =
      (cittResult as (CITCResult & { total_job_mins?: number }) | undefined)
        ?.total_job_mins ??
      analysis?.profitability.total_time_mins ??
      0;
  ...
  const conflicting = useMemo(() => {
    ...
    return analysis.conflictingJobIds
      .map((cid) => map.get(cid))
      .filter(Boolean);
  }, [analysis]);
            ...
            <div className="flex flex-col">
              {[
                ...
                [
                  "Date & time",
                  format(
                    new Date(booking.requested_time),
                    "MMM d, yyyy '·' h:mm a",
                  ),
                ],
                ...
                <div
                  key={label}
                  className="flex flex-col py-2 border-b border-border gap-0.5 last:border-b-0"
                >
          ...
          {isPending && (
            <>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-px bg-border border border-border rounded-[8px] overflow-hidden mb-3">
                      <div className="bg-white text-center p-2.5">
                        <div className="font-inter text-[10px] text-slate-secondary mb-0.5">
                          Fee
                        </div>
                      ...
                      <div className="bg-white text-center p-2.5 border-l border-border">
                        ...
                        <div
                          className={`font-sora text-[16px] font-bold ${merged.verdictClass}`}
                        >
                    ...
                    {(analysisReady || cittReady) && (
                      <div className="flex flex-col gap-1.5 mb-3 pb-3 border-b border-border">
                        ...
                        {cittReady &&
                          (cittResult!.gap_before != null ||
                            cittResult!.gap_after != null) && (
                            <DetailRow
                              label="Schedule gaps"
                              value={[
                                cittResult!.gap_before != null &&
                                  `${cittResult!.gap_before} min before`,
                                cittResult!.gap_after != null &&
                                  `${cittResult!.gap_after} min after`,
                              ]
                                .filter(Boolean)
                                .join(" · ")}
                              valueClass={
                                (cittResult!.gap_before != null &&
                                  cittResult!.gap_before < 0) ||
                                (cittResult!.gap_after != null &&
                                  cittResult!.gap_after < 0)
                                  ? "text-red-danger"
                                  : "text-teal-success"
                              }
                            />
                    ...
                    {/* ── CITT verdict pill ── */}
                    {cittReady && <CITTVerdict result={cittResult!} />}
                        ...
                        <CheckRow
                          ...
                          label={
                            cittResult!.scanback_conflict
                              ? cittResult!.scanback_conflict_detail ||
                                "Scanback window conflict detected"
                    ...
                    {analysisReady && analysis!.dayJobs.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-border">
                        ...
                        {analysis!.dayJobs.map((j) => {
                          const isConflict =
                            analysis!.conflictingJobIds.includes(j.id);
                          return (
                            <div
                              key={j.id}
                        ...
                        {cittReady && cittResult!.next_job && (
                          <div className="flex gap-2 py-1.5 border-t border-border">
                            <span className="font-inter text-[10px] text-muted w-[64px] flex-shrink-0">
                              {format(
                                new Date(cittResult!.next_job.time),
                                "h:mm a",
                              )}
                            </span>
                            <span className="font-inter text-[11px] text-slate">
                              {cittResult!.next_job.type.replace(/_/g, " ")} ·
                              next
                            </span>
                          </div>
                        )}
              ...
              <div className="alert al-blue mb-4">
                <AlertTriangle className="w-4 h-4 text-blue flex-shrink-0 mt-0.5" />
                <div className="font-inter text-[11px] leading-[1.4]">
                  Approving creates a confirmed job and sends a confirmation
                  email to the client.
                  {/* Calendar sync hidden for now — re-enable when ready.
                  and updates your calendar and .ics feed. */}
                </div>
              </div>
            </>
          )}
          ...
          {isPending && !declineOpen && (
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                style={{ color: "var(--red)" }}
              >
                <XCircle className="w-4 h-4" /> Decline and suggest alternative
                times
              </button>
            </div>
          )}
...
function DetailRow({
  ...
  return (
    <div className="flex justify-between items-center">
      ...
      <span className={`font-inter text-[11px] font-semibold ${valueClass}`}>
        {value}
      </span>
    </div>
  );
...
function CheckRow({ ok, label }: { ok: boolean; label: string }) {
  return (
    <div className="flex items-start gap-2">
      ...
      <span className="font-inter text-[11px] text-slate leading-[1.4]">
        {label}
      </span>
    </div>
  );
