...
/**
 * The full public booking page experience, shared by /book/[username] and the
 * authenticated review-mode preview at /booking-preview. In preview mode the
 * page behaves identically (real slots, real profile data) but the submit
 * action is disabled so no booking request is ever sent.
 */
        ...
        <div className="flex-1 px-5 py-6 md:py-9 md:px-8">
          ...
          <div className="flex flex-col gap-4 max-w-[460px]">
            ...
            <button
              onClick={() => {
                if (preview) return;
                submitBooking.mutate(undefined);
              }}
              disabled={
                preview ||
                !form.client_name ||
            ...
            >
              {preview ? (
                <span>Preview mode — booking requests are disabled</span>
              ) : submitBooking.isPending ? (
                "Submitting..."
              ) : (
                <>
            ...
            <p className="font-inter text-[11px] text-muted text-center">
              {preview
                ? "This is a review copy of your live booking page."
                  ...
                  {alts.map((a) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          if (preview) return;
                          submitBooking.mutate(a.iso);
                        }}
                        disabled={preview || submitBooking.isPending}
